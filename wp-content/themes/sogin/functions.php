...
function activate_cron() {
    if( ! wp_next_scheduled( 'get_rates' ) ) {
        wp_schedule_event( time(), 'hourly', 'get_rates');
    }
}

// добавляем функцию к указанному хуку
add_action( 'get_rates', 'do_this_hourly' );
function do_this_hourly() {
    global $wpdb;
    if( $curl = curl_init() ) {
        curl_setopt($curl, CURLOPT_URL, 'http://www.nbrb.by/API/ExRates/Rates/145');
        curl_setopt($curl, CURLOPT_RETURNTRANSFER,true);
        $out = curl_exec($curl);
        $rate =json_decode($out);
        $wpdb->update('rates',array('value'=> $rate->Cur_OfficialRate), array('code' => 'USD'));
        curl_close($curl);
    }
}

function get_price($original, $rate){
    return 'от ' . round($original * $rate, 2) . ' руб.';
}